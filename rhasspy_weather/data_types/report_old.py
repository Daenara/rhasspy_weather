...
class WeatherReport:
        ...
        if self.__request.detail:
            ...
            for fixed_time in [FixedTimes.MORNING, FixedTimes.AFTERNOON, FixedTimes.EVENING]:
                ...
                if weather is not None:
                    self.weather[fixed_time.value] = weather
            ...
            self.weather[(datetime.time.min, datetime.time.max)] = self.__forecast.weather_for_day(self.__request.request_date)
            response = self.__answer_condition(self.weather[Grain.DAY]).format(when=self.__output_date_and_time, where=self.__output_location)
            response = response + " " + self.__answer_temperature(self.weather[FixedTimes.DAY].min_temperature, self.weather[FixedTimes.DAY].max_temperature).format(when="", where="")
        return response
    ...
    def __generate_temperature_report_day(self):
        ...
        if self.__request.detail and self.__request.requested is None:
            ...
            for fixed_time in [FixedTimes.MORNING, FixedTimes.AFTERNOON, FixedTimes.EVENING]:
                weather = self.__forecast.weather_for_interval(self.__request.request_date, fixed_time.value[0], fixed_time.value[1])
                if weather is not None:
                    self.weather[fixed_time] = weather
            ...
            self.weather[FixedTimes.DAY] = self.__forecast.weather_for_day(self.__request.request_date)
            response = self.__answer_temperature(self.weather[FixedTimes.DAY].min_temperature, self.weather[FixedTimes.DAY].max_temperature).format(when=self.__output_date_and_time, where=self.__output_location)
        return response
    ...
    def __generate_condition_report_day(self):
        ...
        if self.__request.detail:
            ...
            self.weather[FixedTimes.DAY] = self.__forecast.weather_for_day(self.__request.request_date)
            response = self.__answer_condition(self.weather[FixedTimes.DAY], True).format(when=self.__output_date_and_time, where=self.__output_location)
        return response
